...
Docker_Hub_username="claychengaws"
...
publish_to_docker_hub() {
	...
	if [ $# -eq 2 ]; then
		...
		do
			docker tag ${1}:"$arch" claychengaws/aws-for-fluent-bit:"${arch}"-${AWS_FOR_FLUENT_BIT_VERSION}
			docker push claychengaws/aws-for-fluent-bit:"$arch"-${AWS_FOR_FLUENT_BIT_VERSION}

			docker tag ${1}:"$arch"-"debug" claychengaws/aws-for-fluent-bit:"${arch}"-"debug"-${AWS_FOR_FLUENT_BIT_VERSION}
			docker push claychengaws/aws-for-fluent-bit:"$arch"-"debug"-${AWS_FOR_FLUENT_BIT_VERSION}
			
			docker tag ${1}:"$init"-"$arch" claychengaws/aws-for-fluent-bit:"$init"-"${arch}"-${AWS_FOR_FLUENT_BIT_VERSION}
			docker push claychengaws/aws-for-fluent-bit:"$init"-"$arch"-${AWS_FOR_FLUENT_BIT_VERSION}

			docker tag ${1}:"$init"-"$arch"-"debug" claychengaws/aws-for-fluent-bit:"$init"-"${arch}"-"debug"-${AWS_FOR_FLUENT_BIT_VERSION}
			docker push claychengaws/aws-for-fluent-bit:"$init"-"$arch"-"debug"-${AWS_FOR_FLUENT_BIT_VERSION}

		done

		create_manifest_list claychengaws/aws-for-fluent-bit "latest" ${AWS_FOR_FLUENT_BIT_VERSION}
		create_manifest_list claychengaws/aws-for-fluent-bit ${AWS_FOR_FLUENT_BIT_VERSION} ${AWS_FOR_FLUENT_BIT_VERSION}

		create_manifest_list_init claychengaws/aws-for-fluent-bit "init-latest" ${AWS_FOR_FLUENT_BIT_VERSION}
		create_manifest_list_init claychengaws/aws-for-fluent-bit "$init"-${AWS_FOR_FLUENT_BIT_VERSION} ${AWS_FOR_FLUENT_BIT_VERSION}
	fi
}
...
publish_to_public_ecr() {
	if [ $# -eq 2 ]; then
		...
		docker pull public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:stable || echo "0"
		sha1=$(docker inspect --format='{{index .RepoDigests 0}}' public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:stable || echo "0")
		docker pull public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:${AWS_FOR_FLUENT_BIT_STABLE_VERSION}
		sha2=$(docker inspect --format='{{index .RepoDigests 0}}' public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:${AWS_FOR_FLUENT_BIT_STABLE_VERSION})

		match_two_sha $sha1 $sha2

		if [ "$IMAGE_SHA_MATCHED" = "FALSE" ]; then
			aws ecr-public get-login-password --region us-east-1 | docker login --username AWS --password-stdin public.ecr.aws/${public_ecr_custom_alias}
			create_manifest_list public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit "stable" ${AWS_FOR_FLUENT_BIT_STABLE_VERSION}
		fi
	else
		aws ecr-public get-login-password --region us-east-1 | docker login --username AWS --password-stdin public.ecr.aws/${public_ecr_custom_alias}
		...
		for arch in "${ARCHITECTURES[@]}"
		do
			docker tag ${1}:"$arch" public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:"$arch"-${AWS_FOR_FLUENT_BIT_VERSION}
			docker push public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:"$arch"-${AWS_FOR_FLUENT_BIT_VERSION}

			docker tag ${1}:"$arch"-"debug" public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:"$arch"-"debug"-${AWS_FOR_FLUENT_BIT_VERSION}
			docker push public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:"$arch"-"debug"-${AWS_FOR_FLUENT_BIT_VERSION}

			docker tag ${1}:"$init"-"$arch" public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:"$init"-"$arch"-${AWS_FOR_FLUENT_BIT_VERSION}
			docker push public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:"$init"-"$arch"-${AWS_FOR_FLUENT_BIT_VERSION}

			docker tag ${1}:"$init"-"$arch"-"debug" public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:"$init"-"$arch"-"debug"-${AWS_FOR_FLUENT_BIT_VERSION}
			docker push public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:"$init"-"$arch"-"debug"-${AWS_FOR_FLUENT_BIT_VERSION}
		done

		create_manifest_list public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit ${AWS_FOR_FLUENT_BIT_VERSION} ${AWS_FOR_FLUENT_BIT_VERSION}
		aws ecr-public get-login-password --region us-east-1 | docker login --username AWS --password-stdin public.ecr.aws/${public_ecr_custom_alias}
		create_manifest_list public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit "latest" ${AWS_FOR_FLUENT_BIT_VERSION}

		create_manifest_list_init public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit "$init"-${AWS_FOR_FLUENT_BIT_VERSION} ${AWS_FOR_FLUENT_BIT_VERSION}
		aws ecr-public get-login-password --region us-east-1 | docker login --username AWS --password-stdin public.ecr.aws/${public_ecr_custom_alias}
		create_manifest_list_init public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit "init-latest" ${AWS_FOR_FLUENT_BIT_VERSION}
	fi
}
...
sync_public_and_repo() {
	...
	docker pull public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:${tag}
	sha1=$(docker inspect --format='{{index .RepoDigests 0}}' public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:${tag})
	...
	if [ "$IMAGE_SHA_MATCHED" = "FALSE" ]; then
		...
		push_image_ecr public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:${tag} \
...
sync_latest_image() {
	...
	for arch in "${ARCHITECTURES[@]}"
	do
		aws ecr-public get-login-password --region us-east-1 | docker login --username AWS --password-stdin public.ecr.aws/${public_ecr_custom_alias} || echo "0"
...
check_image_version() {
	...
	docker manifest inspect public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:${1} > /dev/null || EXIT_CODE=$?
	if [ "${EXIT_CODE}" = "0" ]; then
...
verify_public_ecr() {
	...
	if [ $# -eq 1 ]; then
		# Get the image SHA's
		docker pull public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:stable
		sha1=$(docker inspect --format='{{index .RepoDigests 0}}' public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:stable)
		docker pull public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:${AWS_FOR_FLUENT_BIT_STABLE_VERSION}
		sha2=$(docker inspect --format='{{index .RepoDigests 0}}' public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:${AWS_FOR_FLUENT_BIT_STABLE_VERSION})
		...
		# Get the image SHA's
		docker pull public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:latest
		sha1=$(docker inspect --format='{{index .RepoDigests 0}}' public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:latest)
		docker pull public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:${AWS_FOR_FLUENT_BIT_VERSION}
		sha2=$(docker inspect --format='{{index .RepoDigests 0}}' public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:${AWS_FOR_FLUENT_BIT_VERSION})

		verify_sha $sha1 $sha2

		docker pull public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:"$init"-latest
		sha1_init=$(docker inspect --format='{{index .RepoDigests 0}}' public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:"$init"-latest)
		docker pull public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:"$init"-${AWS_FOR_FLUENT_BIT_VERSION}
		sha2_init=$(docker inspect --format='{{index .RepoDigests 0}}' public.ecr.aws/${public_ecr_custom_alias}/amazon/aws-for-fluent-bit:"$init"-${AWS_FOR_FLUENT_BIT_VERSION})
